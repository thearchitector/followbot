...
//        fs["Q"] >> Q;
    ...
    for (int i=0; i < xyz.rows; i++) {
        for (int j=0; j < xyz.cols; j++) {
            if (xyz.at<Point3f>(i, j).z < 20) {
                buffer.push_back(xyz.at<Point3f>(i, j));
            }
        }
    }
    ...
    viz::WLine linei(O, i, viz::Color::white());
    viz::WLine linej(O, j, viz::Color::red());
    viz::WLine linek(O, k, viz::Color::blue());

    viz::Viz3d myWindow("Coordinate Frame");
    viz::WCloud cloud_widget = viz::WCloud(buffer, viz::Color::green());
