...
void AStar::showPersonPathAndHeading() {
    if (!buggerWindow.wasStopped()) {
        ...
        for (int i=0; i < path.size() - 1; ++i) {
            auto arrow = cv::viz::WArrow({static_cast<double>(path[i].first), 0, static_cast<double>(path[i].second)},
                    {static_cast<double>(path[i+1].first), 0, static_cast<double>(path[i+1].first)}, 0.03, cv::viz::Color::green());
...
void AStar::findAStarPath() {
    // note: path is already cleared before this function call
    ...
    while (!openList.empty() && counter < MAX_ASTAR_LOOPS) {
        ...
        openMap.insert({{itNode->first, itNode->second}, false});  // mark the current node as not open in the openMap
            ...
            for (int deltaY = -1; deltaY <= 1; ++deltaY) {
                ...
                if (isUnBlocked(newNode)) {  // if the adjacent node is unblocked
                    ...
                    if (isDestination(newNode)) {
                        ...
                        if (gNew < g_at_neighbor->second || g_at_neighbor == gMap.end()) {  // this is the best path seen yet - record it
                            cameFrom.insert({newNode, currentNode});
