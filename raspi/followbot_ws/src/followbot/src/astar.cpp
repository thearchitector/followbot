...
void AStar::showPersonPathAndHeading() {
    if (!buggerWindow.wasStopped()) {
        ...
        int node_counter = 0;
        for (auto &i : path) {
            auto arrow = cv::viz::WArrow({static_cast<double>(i.parent_x), 0, static_cast<double>(i.parent_z)},
                    {static_cast<double>(i.x), 0, static_cast<double>(i.z)}, 0.03, cv::viz::Color::green());
            buggerWindow.showWidget("line" + std::to_string(node_counter), arrow);
            ++node_counter;
        }
...
//    current_heading = (short)std::round(std::atan2(world_msg->person.x, world_msg->person.z));
    current_person_int = IntPair{floor(world_msg->person.x / OCCUPANCY_GRID_SCALE),
    ...
    if (person_is_found) {
        ...
        path = {AStarNode{ROBOT_POSE.first, ROBOT_POSE.second, ROBOT_POSE.first, ROBOT_POSE.second, 0., 0., 0.}};
        #ifndef PRODUCTION
        fillOccupanyGrid(world_msg);
        showPersonPathAndHeading();
        #endif
...
void AStar::findAStarPath() {
    // TODO: change cost depending on straight or diagonal step
    ...
    if (isDestination(IntPair{0, 0}, dest_person_int)) {
        ...
        path = {AStarNode{dest_person_int.first, dest_person_int.second, ROBOT_POSE.first, ROBOT_POSE.second, 0., 0.,
                          0.}};
        return;
    }
    ...
    int x = 0;
    int z = 0;
    allMap.insert({IntPair(x, z), AStarNode{x, z, x, z, 0., 0., 0.}});
    openList.emplace_back(allMap.find(IntPair(x, z))->second);
    ...
    while (!openList.empty() && counter < MAX_ASTAR_LOOPS) {
        ...
        } while (!isUnBlocked(IntPair{node.x, node.z}));

        x = node.x;
        z = node.z;
        closedList.insert({IntPair{x, z}, true});
                ...
                if (isUnBlocked(newIntPair)) {
                    ...
                    if (isDestination(IntPair{x + newX, z + newY}, dest_person_int)) {
                        ...
                        if (allMap.find(newIntPair) == allMap.end()) {
                            allMap.insert(
                                    {newIntPair,
                                     AStarNode{newIntPair.first, newIntPair.second, x, z, FLT_MAX, FLT_MAX, FLT_MAX}});
                        }
    ...
    std::cout << " A*: Destination not found (with " << counter << " loops); returning equivalent of src == dst"
              << std::endl;
...
void AStar::makePath(std::map<IntPair, AStarNode> &allMap, const IntPair &dest) {
    ...
    while (true) {
        auto foundAtXY = allMap.find(IntPair{x, z});
        if (foundAtXY != allMap.end()) {
            AStarNode nodeFoundAtXY = foundAtXY->second;
            if (!(nodeFoundAtXY.parent_x == x && nodeFoundAtXY.parent_z == z)) {
                intermediate_path.push(allMap.find(IntPair{x, z})->second);
                int tempX = nodeFoundAtXY.parent_x;
                int tempY = nodeFoundAtXY.parent_z;
                x = tempX;
                z = tempY;
            } else {
                break;
    ...
    while (!intermediate_path.empty()) {
        ...
        std::cout << "(" << top.x << ", " << top.z << ") ";  // print out path
    }
}
